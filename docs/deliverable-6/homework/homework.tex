...
%task 6
% There are two main categories of lines codes; return-to-zero (RZ) and non-return-to-zero (NRZ). RZ signals return to zero halfway during the transmission of a single bit and attain their new value at the end. NRZ signals do not exhibit this behaviour, they retain their value. The other important aspect of line codes is the polarity. Signals can be unipolar ($s(t) \in \{0,A\}$), polar ($s(t) \in \{-A,A\}$) and bipolar ($s(t) \in \{-A,0,A\}$). The most used line codes are punch tape code, unipolar NRZ, polar NRZ, unipolar RZ, bipolar RZ and Manchester coding, which is neither RZ or NRZ.
