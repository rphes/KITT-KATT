...
\subsection{Report 1}
\label{ass2-report-1}
As calculated in paragraph \ref{ass1-rep-12-13} the sampling frequency should be $f=\SI{34}{\kilo\hertz}$ in order to obtain a \SI{1}{\centi\meter} spatial resolution. To satisfy the Nyquist criterion, the microphones must sample at twice this frequency (e.g. $f=\SI{68}{\kilo\hertz}$) to avoid aliasing. This seems a bit high and might lead to problems both in storage of samples (5 channels) and processing power. Therefore, sampling at \SI{44.1}{\kilo\hertz} seems more realistic and this sampling rate is commonly supported by audio interfaces. The (theoretical) highest frequency that can be detected accurately is then \SI{22.05}{\kilo\hertz}, although due to the need of a low pass filter and corresponding transition band, \SI{20}{\kilo\hertz} is accomplished in practice \footnote{See \url{http://www.cs.columbia.edu/~hgs/audio/44.1.html} for details on why \SI{44.1}{\kilo\hertz} is dominant.}, corresponding to \SI{1.7}{\centi\meter}; this seems reasonable. The maximum propagation delay corresponding to \SI{5}{\meter} is $t_{\text{5m}}=\frac{5}{340}=\SI{147}{\milli\second}$. This corresponds to \num{649} samples at \SI{44.1}{\kilo\hertz}, thus defining the peak search interval.
...
Explanation of peak detection algorithm here.
...
An example of a received data segment is shown in figure \ref{fig:ass-2-rep-2-data-segment}. For details on the peak detection algorithm, refer to section \ref{ass2-report-1}. For an example of the location of the peak as detected by our software see section \ref{ass2-report-4}.
